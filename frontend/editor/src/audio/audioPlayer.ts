...
deriveDebounced(volumeStore, 0.5).subscribe(volume => {
  if(volume !== undefined) sendToast(`Volume: ${volume}%`)
});
...
deriveDebounced(rateStore, 0.5).subscribe(rate => {
  if(rate !== undefined) sendToast(`Playback Rate: ${rate}%`)
});
